...
def add_to_cart(request, product_id):
    ...
    if quantity > product.stock:
        messages.error(
            request, f"Sorry, only {product.stock} units of {product.name} available!"
        )
    ...
    if current_qty + quantity > product.stock:
        messages.error(
            request,
            f"Cannot add {quantity} more. Only {product.stock - current_qty} available in stock!",
        )
        return redirect("product_detail", product_id=product_id)
        ...
        if quantity <= 0:
            ...
            if quantity > product.stock:
                if request.headers.get("X-Requested-With") == "XMLHttpRequest":
                    ...
                    messages.error(
                        request,
                        f"Sorry, only {product.stock} units of {product.name} available!",
                    )

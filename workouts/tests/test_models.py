...
class ExerciseModelTest(TestCase):
    def setUp(self):
        ...
        self.exercise = Exercise.objects.create(
            workout=self.workout, name="Push Ups", sets=3, reps="12", rest_seconds=60
        )
...
class ExerciseProgressModelTest(TestCase):
    def setUp(self):
        ...
        self.exercise = Exercise.objects.create(
            workout=self.workout, name="Squats", sets=3, reps="10", rest_seconds=45
        )
        self.progress = ExerciseProgress.objects.create(
            user=self.user, exercise=self.exercise, completed=False
        )
    ...
    def test_unique_together_constraint(self):
        ...
        with self.assertRaises(Exception):
            ExerciseProgress.objects.create(
                user=self.user, exercise=self.exercise, completed=True
            )
